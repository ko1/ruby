...
ractor_sync_mark(rb_ractor_t *r)
{
     ...
     * locked_by bookkeeping inappropriate for the GC) only during a local GC: in a STW
...
ractor_basket_new(rb_execution_context_t *ec, VALUE obj, enum ractor_basket_type type, bool exc)
{
     ...
     * from the receiver's basket queue -- a cross-objspace edge both local GCs skip. Pin
...
ractor_basket_accept(struct ractor_basket *b)
{
     ...
     * Otherwise it stays physically in the sender, whose local GC would reclaim it once
