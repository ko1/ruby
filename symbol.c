...
set_id_entry(rb_symbols_t *symbols, rb_id_serial_t num, VALUE str, VALUE sym)
{
    ...
    if (idx >= (size_t)RARRAY_LEN(ids) || NIL_P(id_entry_list = rb_ary_entry(ids, (long)idx))) {
        ...
        /* Reached from all Ractors via symbols->ids, but allocated in the
         * interning Ractor's objspace where no local root reaches it. Mark
         * it shareable so the owning objspace's local GC pins it. */
