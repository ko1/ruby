...
//#define TRANSIENT_HEAP_TOTAL_SIZE  (TRANSIENT_HEAP_BLOCK_SIZE * 2) // (1024 * 1024 *   16)
...
transient_heap_block_alloc(struct transient_heap* theap)
{
...
#else
    if (theap->arena == NULL) {
        int err = posix_memalign((void **)&theap->arena, TRANSIENT_HEAP_BLOCK_SIZE, TRANSIENT_HEAP_TOTAL_SIZE);
        if (err != 0) rb_bug("transient_heap_block_alloc: posix_memalign error: %d", err);
    }
    ...
    // fprintf(stderr, "transient_heap_block_alloc: %4d %p\n", theap->total_blocks, block);
    return block;
}
	...
	if (header->magic != TRANSIENT_HEAP_ALLOC_MAGIC) rb_bug("rb_transient_heap_mark: wrong header %s\n", rb_obj_info(obj));
        ...
        if (obj != Qnil) {
            switch (BUILTIN_TYPE(obj)) {
              case T_ARRAY:
#if TRANSIENT_HEAP_DEBUG_DONT_PROMOTE
                rb_ary_transient_heap_promote(obj, FALSE);
#else
...
transient_heap_escape(void *dmy)
{
    ...
    if (theap->status == transient_heap_marking) {
        ...
        if (TRANSIENT_HEAP_DEBUG >= 1) {
            int i;
            fprintf(stderr, "!! transient_heap_escape start total_blocks:%d\n", theap->total_blocks);
